...
class DrawLevels(object):
    ...
    def __init__(self, data):
        ...
        if self.title is not False:
            self.height += SPAN_TITLE * self.y_resolution
        ...
        self.font_title = ImageFont.truetype(self.font_dir, int(self.y_resolution * SPAN_TITLE / 1.3))
    ...
    def draw_level(self):
        ...
        if self.title is not False:
            self.y_cursor += (SPAN_TITLE / 2) * self.y_resolution
            draw_support.draw_text_in_center(
                self.draw,
                self.width / 2,
                self.y_cursor,
